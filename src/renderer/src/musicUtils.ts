...
import { Song, Section, Line, Chord } from './types'
...
export function incrementNote(note: string): string {
  const idx = normalizeNote(note)
  return notes[(idx + 1) % notes.length]
}

export function decrementNote(note: string): string {
  const idx = normalizeNote(note)
  return notes[(idx - 1 + notes.length) % notes.length]
}
